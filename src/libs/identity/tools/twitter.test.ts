...
describe("Twitter Scraper", () => {
    ...
    test("should extract tweet from valid URL", async () => {
        ...
        const result = await twitter.getTweetByUrl(tweetUrl)
    ...
    test("should handle twitter.com URL", async () => {
        const tweetUrl =
            "https://twitter.com/tcookingsenpai/status/1938517728379167227"
    ...
    test("should handle invalid URL gracefully", async () => {
        ...
        expect(twitter.getTweetByUrl(invalidUrl)).rejects.toThrow(
            "Invalid tweet URL",
        )
    })
    ...
    test("should return null for non-existent tweet", async () => {
        const nonExistentUrl =
            "https://x.com/tcookingsenpai/status/9999999999999999999"

        const result = await twitter.getTweetByUrl(nonExistentUrl)
    ...
    test("should work with retry function", async () => {
        ...
        const result = await twitter.getTweetByUrlWithRetry(tweetUrl, 2, 1000)
    ...
    test("should handle consecutive tweet fetching and measure rate limits", async () => {
        ...
        for (let i = 0; i < testTweets.length; i++) {
            ...
            try {
                console.log(
                    `📋 Request ${i + 1}/${testTweets.length}: ${
                        testTweets[i]
                    }`,
                )
                const result = await twitter.getTweetByUrl(testTweets[i])
                ...
                console.log(
                    `✅ Request ${i + 1} completed in ${duration}ms - ${
                        result ? "SUCCESS" : "NULL"
                    }`,
                )
                ...
                timings.push(duration)
                console.log(
                    `❌ Request ${i + 1} failed in ${duration}ms - ${
        ...
        console.log(
            `Requests per minute (theoretical): ${(60000 / avgTime).toFixed(
                2,
            )}`,
        )
        ...
        results.forEach((result, i) => {
            console.log(
                `Request ${i + 1}: ${result.success ? "✅" : "❌"} ${
                    result.duration
                }ms ${result.text || result.error || ""}`,
            )
        })
    ...
    test("should handle burst requests and identify rate limiting", async () => {
        ...
        console.log(
            `🚀 Testing burst requests (${burstCount} requests to same URL)...`,
        )

        const startTime = Date.now()
        const promises = Array.from({ length: burstCount }, (unused, i) =>
            twitter
                .getTweetByUrl(sameUrl)
                .then(result => ({
                    requestNumber: i + 1,
                    success: result !== null,
                    timestamp: Date.now(),
        ...
        responses.forEach((response, i) => {
            if (response.status === "fulfilled") {
                ...
                console.log(
                    `Request ${i + 1}: ❌ REJECTED - ${response.reason}`,
                )
            }
        })
        ...
        console.log(`Total time: ${totalTime}ms`)
        console.log(
            `Requests per second: ${(burstCount / (totalTime / 1000)).toFixed(
                2,
            )}`,
        )
        ...
        if (successCount < burstCount) {
            console.log(
                `⚠️  Rate limiting detected: ${(
                    ((burstCount - successCount) / burstCount) *
                    100
